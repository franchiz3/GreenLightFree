...
public class dreamloLeaderBoard : MonoBehaviour {
    ...
    public bool NameExists(string name)// check the leaderboard for the name since I'm too lazy to make a real sign in system
    {
        string[] names = ToStringArray();
        if (names == null)
            return false;
        ...
        {
            string[] values = names[x].Split(new char[] { '|' }, System.StringSplitOptions.None);
            if (values[0] == name)
                return true;
        }
        return false;
    }
    public static void SwitchLinks(int setting)
    {
        if (setting == 30)
        {
            ButtonMaster.dl.privateCode = "IF5tOhLL30qsENSU3WumkAjmmwtH5LnESCydNnA4jSaQ";
            ButtonMaster.dl.publicCode = "5c0ade0cb6397e00e0fa0278";
        }
        else if (setting == 60)
        {
            ButtonMaster.dl.privateCode = "pHeaqo7XFUKGCcobPMrKtAJayrXgYIOk2Ku8LWd7fn0A";
            ButtonMaster.dl.publicCode = "5c0b1a0cb6397e00e0fad9a4";
        }
        else
        {
            ButtonMaster.dl.privateCode = "qL3VnOvWhk-1bKDe7UwsUgIr5-99hCLUy5jN4crvmhqA";
            ButtonMaster.dl.publicCode = "5c0b19dcb6397e00e0fad908";
        }
    }
